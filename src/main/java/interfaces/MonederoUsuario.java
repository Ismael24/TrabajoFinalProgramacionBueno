...
/**
 * Clase que representa la ventana del monedero personal del usuario, comentaré lo destacado, el resto es diseño visual de la ventana.
	...
	public MonederoUsuario(Ventana v) {
		...
		botonPagar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				// a través del botón pagar vamos a añadir a la cuenta de nuestro usuario el importe que quiera siempre y cuando sea igual o superior a 5 euros
				try {
					if (dineroAñadir.getText().equals("") || Float.parseFloat(dineroAñadir.getText()) < 5.0) {
						JOptionPane.showMessageDialog(ventana, "Añada un importe superior a 5 euros", "Error",
								JOptionPane.INFORMATION_MESSAGE);
					} else {
						ventana.monederoActual.setSaldo((float) ((ventana.monederoActual.getSaldo())
								+ (Float.parseFloat(dineroAñadir.getText()))));
						Connection conexion = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/dropgames",
								"root", "admin");

						Statement smta = conexion.createStatement();

						smta.executeUpdate("UPDATE monedero SET saldo = '" + ventana.monederoActual.getSaldo()
								+ "' WHERE nombreUsuario = '" + ventana.usuarioLogado.getNombre() + "'");

						labelSaldoActual.setText("" + ventana.monederoActual.getSaldo());

						smta.close();
						conexion.close();
						JOptionPane.showMessageDialog(ventana, "Pago realizado con éxito", "Éxito",
								JOptionPane.INFORMATION_MESSAGE);
