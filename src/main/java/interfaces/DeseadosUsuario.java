...
"jdbc:mysql://127.0.0.1:3306/dropgames","root","admin"
			...
			while(resultSetJuegosUsuario.next()) {
				...
				try {
					...
					final ResultSet resultSetJuegos=smta.executeQuery(
							"select * from juego"
							);
						...
						botonQuitarDeseados.addMouseListener(new MouseAdapter() {
							...
							public void mouseClicked(MouseEvent e) {
								try {
									...
									smta.close();
									conexion.close();
									JOptionPane.showMessageDialog(ventana, "El juego se ha quitado de la lista de deseados","Éxito",JOptionPane.INFORMATION_MESSAGE);
